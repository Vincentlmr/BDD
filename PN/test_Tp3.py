...
from TP3.py import eval_Neuville

# On écrit une fonction Python par test unitaire
# Le contenu du test doit bien sûr être adapté au cas par cas

def test_eval_Neuville_1():
    A = np.array ([], dtype=np.float64)
    assert (eval_Neuville (17, A) == 0), "0 attendu"

def test_eval_Neuville_2():
    A = np.array ([-1, 0, 1], dtype=np.float64)
    assert (eval_Neuville (-1, A) == 0), "0 attendu"

def test_eval_Neuville_3():
    A = np.array ([-1, 0, 1], dtype=np.float64)
    assert (eval_Neuville (1, A) == 0), "0 attendu"

# La liste L des fonctions définies ci-dessus (à adapter !)

L = [test_eval_Neuville_1, test_eval_Neuville_2, test_eval_Neuville_3]
